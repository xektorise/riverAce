...
class Hand:
    ...
    def _is_flush(self, suits):
        """Check if the hand is a flush and return the suit if true."""
        suit_counts = Counter(suits)
    ...
    def _get_highest_hand(self, ranks, counts, num, flush=False, flush_suit=None):
        """Get the highest hand based on the ranks and counts."""
        if flush:
            return sorted([rank for rank in ranks if flush_suit], reverse=True)[:num]
        return [rank for rank in ranks if counts[rank] == 1][:num]

    def _get_rank_hand(self, rank_counts, count, pair_count=1):
        """Get the specific rank hand based on the count."""
